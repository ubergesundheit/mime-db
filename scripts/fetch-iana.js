...
var EXTENSIONS_REGEXP = /(?:^\s*(?:\d\.\s+)?|\s+[23]\.\s+)File extension(?:\(s\)|s|)\s?:\s+(?:\*\.|\.|)([0-9a-z_-]+)(?:(?:\s+or|)\s+(?:\*\.|\.|)([0-9a-z_-]+)|\s*,\s*(?:\*\.|\.|)([0-9a-z_-]+)(?:\s*,\s*(?:\*\.|\.|)([0-9a-z_-]+))?(?:\s*,\s*(?:\*\.|\.|)([0-9a-z_-]+))?)?\s*(?:\(|[34]\.\s+|$)/im
var MIME_TYPE_HAS_CHARSET_PARAMETER_REGEXP = /parameters\s*:[^.]*\bcharset\b/im

co(function * () {
  var gens = yield [
    get('application', { extensions: /(?:\/(?:gzip|ld\+json|n-quads|n-triples|vnd\.apple\..+|vnd\.rar)|\+xml)$/ }),
